...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function displayWebtoonData(data) {
        ...
        // const line10Labels = recentEpisodes10.map(episode => `${episode.episode}`);
        ...
        // const line100Labels = recentEpisodes100.map(episode => `${episode.episode}`);
        ...
        // const allLabels = allEpisodes.map(episode => `${episode.episode}`);
        // const allValues = allEpisodes.map(episode => episode.star);
        // drawAllLineChart(allLabels, allValues);

         // 데이터 가져와서 선그래프 그리기 (최근 10화)
         if (data.last_ep > 10) {
            document.getElementById("recent10Episodes").style.display = "block";
            document.getElementById("recent10EpisodesGraph").style.display = "block";

            const sortedStars10 = data.stars.star_list.sort((a, b) => b.episode - a.episode); 
            const recentEpisodes10 = sortedStars10.slice(0, 10).reverse(); 
            const line10Labels = recentEpisodes10.map(episode => `${episode.episode}`);
            const line10Values = recentEpisodes10.map(episode => episode.star);
            draw10LineChart(line10Labels, line10Values);
        } else {
            document.getElementById("recent10Episodes").style.display = "none";
            document.getElementById("recent10EpisodesGraph").style.display = "none";
        }

        // 데이터 가져와서 선그래프 그리기 (최근 100화)
        if (data.last_ep > 100) {
            document.getElementById("recent100Episodes").style.display = "block";
            document.getElementById("recent100EpisodesGraph").style.display = "block";

            const sortedStars100 = data.stars.star_list.sort((a, b) => b.episode - a.episode); 
            const recentEpisodes100 = sortedStars100.slice(0, 100).reverse(); // 최근 100화만 선택
            const line100Labels = recentEpisodes100.map(episode => `${episode.episode}`);
            const line100Values = recentEpisodes100.map(episode => episode.star);
            draw100LineChart(line100Labels, line100Values);
        } else {
            document.getElementById("recent100Episodes").style.display = "none";
            document.getElementById("recent100EpisodesGraph").style.display = "none";
        }

        // 데이터 가져와서 전체 회차 선그래프 그리기
        const allEpisodes = data.stars.star_list.reverse(); 
        const allLabels = allEpisodes.map(episode => `${episode.episode}`);
        const allValues = allEpisodes.map(episode => episode.star);
        drawAllLineChart(allLabels, allValues);
